...
def test_export_integration() -> None:
    ...
    export_response = client.post("/exports", json=export_payload)
    ...
    get_job_response = client.get(f"/exports/{job_id}")
    assert get_job_response.status_code == 200
    job_status = get_job_response.json()
    assert job_status["status"] == "succeeded"
    
    # 4. Download File
    download_response = client.get(f"/exports/{job_id}/download")
