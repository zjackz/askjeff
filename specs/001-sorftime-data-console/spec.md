...
| import_strategy | Enum('overwrite','append','update_only') | 导入策略 |
...
| failure_summary | JSONB | 失败行文件路径、错误类型计数 |
| columns_seen | JSONB | 本次导入实际出现的列列表 |
| archived | Boolean | 是否被归档 |
...
|------|------|------|
| id | UUID | 导出任务唯一标识 |
| export_type | Enum('clean_products','failed_rows') | 导出类型 |
...
| action | Text | 操作类型 (import.create, chat.ask, export.download 等) |
