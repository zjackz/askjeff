...
- [~] T0YC [P] ~~实现主题切换（亮/暗或品牌主题）~~ **已跳过**
- [~] T0YD ~~基于角色的权限路由/菜单守卫~~ **已跳过**
- [~] T0YE [P] ~~Playwright/组件测试覆盖主题切换与权限守卫可用性~~ **已跳过**
...
- [~] T0XZ [US1] ~~导入性能基准：50MB/10 万行 k6/pytest~~ **已跳过**
...
- [~] T0YA [US2] ~~问答性能与降级验证：P90 ≤10s~~ **已跳过**
...
- [~] T0YB [US3] ~~导出性能与分段测试：50k+ 行 95% ≤2 分钟~~ **已跳过**
