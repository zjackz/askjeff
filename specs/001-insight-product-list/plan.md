...
**Branch**: `001-insight-product-list` | **Date**: 2025-11-18 | **Spec**: specs/001-insight-product-list/spec.md  
**Input**: `/specs/001-insight-product-list/spec.md`

## 摘要

- 目标：将数据洞察页面主区域改为上传产品列表，提供完整筛选/排序/分页与行详情，同时保留右下角悬浮 chat 弹窗交互。
- 方案：前端在现有 Vue 3 + Element Plus 页面中重构主容器与过滤器，复用后端产品查询接口与 chat 交互，新增必要的契约文档与观测埋点，无需改动数据模型和导入流程。
...
- **测试策略**：前端 `pnpm --prefix frontend lint`，后端 `poetry run ruff check && pytest`；交互验收按 quickstart 步骤手测。
- **部署/目标平台**：开发与生产均由 Docker Compose 管理，dev 端口 5174/8001；遵循“容器优先与环境一致性”。
- **性能&约束**：列表查询 80% 在 3 秒内返回；chat 回复或错误提示 90% 在 10 秒内；页面加载后 chat 入口 1 秒内可见。

## 宪章核对（必须全部满足）

1. **P1 用户意图**：交付产品列表化的数据洞察界面与悬浮 chat 弹窗，实现筛选、详情、问答三种主要操作在同页完成。  
2. **P2 独立价值**：用户故事覆盖 P1 列表筛选、P2 悬浮 chat；现规格有 2 个清晰故事，可独立验收与回滚。  
3. **P3 合约先行**：定义/确认产品列表查询、详情与 chat 交互契约（见 contracts/products.md），若后端路径不符需先对齐。  
...
```text
specs/001-insight-product-list/
├── plan.md
├── research.md
├── data-model.md
├── quickstart.md
